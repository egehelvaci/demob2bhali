...
export default function EditPriceListPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const collectionsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'https://expressjs-postgres-production-6aba.up.railway.app'}/api/collections/`);
      ...
      try {
        ...
        const detailResponse = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'https://expressjs-postgres-production-6aba.up.railway.app'}/api/price-lists/${params.priceListId}`, {
          headers: {
